...
if ( !class_exists( 'WPSL_Frontend' ) ) {
    ...
    class WPSL_Frontend {
        ...
        public function show_store_address( $atts ) {
            ...
            $content = '<div class="wpsl-locations-details" itemscope itemtype="http://schema.org/LocalBusiness">';

            if ( $atts['name'] && $name = get_the_title( $atts['id'] ) ) {
                $content .= '<span itemprop="name"><strong>' . $name . '</strong></span>';
            }

            $content .= '<div class="wpsl-location-address" itemtype="http://schema.org/PostalAddress" itemscope="" itemprop="address">';

            if ( $atts['address'] && $address = get_post_meta( $atts['id'], 'wpsl_address', true ) ) {
                $content .= '<span itemprop="streetAddress">' . esc_html( $address );
            }

            if ( $atts['address2'] && $address2 = get_post_meta( $atts['id'], 'wpsl_address2', true ) ) {
                $content .= '<br/>' . esc_html( $address2 ) . '</span><br/>';
            }
			else {
                $content .= '</span><br/>';
            }
            ...
            foreach ( $address_format as $address_part ) {
                ...
                if ( $address_part != 'comma' && $atts[$address_part] ) {
                    ...
                    if ( $post_meta ) {
						...
						if ( $address_part == "city" ) {
							$itemprop = 'addressLocality';
						}
						elseif ( $address_part == 'state' ) {
							$itemprop = 'addressRegion';
						}
						elseif ( $address_part == 'zip' ) {
							$itemprop = 'postalCode';
						}
						else {
							$itemprop = '';
                        ...
                        $content .= '<span itemprop="' . $itemprop . '">' . esc_html( $post_meta ) . $punctuation . '</span>' . $br;
                    }
                }
            ...
            if ( $atts['phone'] || $atts['fax'] || $atts['email'] || $atts['url'] ) {
                ...
                if ( $atts['clickable_contact_details'] ) {
                    $contact_details = array(
                        'phone' => '<a href="tel:+1' . str_replace('-','',$phone) . '" itemprop="telephone">' . esc_html( $phone ) . '</a>',
